...
function buildRungSessions(allGames: Game[]): RungSession[] {
  ...
  // group by date first (sessions reset each date)
  ...
  for (const date of dates) {
    const rounds = byDate[date].slice().sort((a, b) => new Date(a.created_at || '').getTime() - new Date(b.created_at || '').getTime())
    ...
    const flush = () => {
      ...
      // team scores in this session
      const teamScores: Record<string, number> = { ...scores }

      // player best team (max wins among teams they played in)
      const allTeams = new Set<string>()
      current.forEach(r => {
        allTeams.add(teamKey(r.team1 || []))
        allTeams.add(teamKey(r.team2 || []))
      })

      const playerBest: RungSession['playerBest'] = {}
      const players = new Set<string>()
      Array.from(allTeams).forEach(tk => tk.split('&').forEach(p => players.add(p)))
      ...
      players.forEach(p => {
        ...
        if (bestTeamKey) playerBest[p] = { teamKey: bestTeamKey, wins: bestWins }
        else playerBest[p] = { teamKey: '', wins: 0 }
      })

      const tiers = computePlayerTiersFromBest(playerBest)

      const endAtIso = current[current.length - 1]?.created_at || null
      const maxScore = Math.max(...Object.values(teamScores).map(v => v ?? 0), 0)
      const isComplete = maxScore >= 5

      sessions.push({
        key: `${date}-${endAtIso || current[0].id}`,
        gameDate: date,
        rounds: current,
        teamScores,
        playerBest,
    ...
    for (const r of rounds) {
      ...
      if (!(t1 in scores)) scores[t1] = 0
      if (!(t2 in scores)) scores[t2] = 0

      // winning_team refers to team1/team2 as stored in this round
      if (r.winning_team === 1) scores[t1] += 1
      else if (r.winning_team === 2) scores[t2] += 1
  ...
  // Non-round “ONGOING” placeholders (rows without winning_team) are not included in sessions; admin logs rounds anyway.
  return sessions
}
